...
class CompleteSavingSpinUseCaseTest {
    ...
    fun `cash destination completes without wallet transfer or expense creation`() = runTest {
        ...
        coVerify(exactly = 0) { transactionRepository.transferBetweenWallets(any(), any(), any(), any(), any()) }
    ...
    fun `bank transfer with sufficient balance executes transfer and completes session`() = runTest {
        ...
        coEvery {
            transactionRepository.transferBetweenWallets("wallet_src", "wallet_dest", 50_000, any(), now)
        } returns AppResult.Success(Unit)
        coEvery {
            repository.completeSession("day:2026-09-04", "mb_dest", SavingMethod.BANK_TRANSFER, "saving-spin:day_2026-09-04")
        } returns AppResult.Success(Unit)
