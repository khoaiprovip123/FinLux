    ...
    Box(Modifier.fillMaxSize()) {
        ...
        Scaffold(
            ...
            LazyColumn(
                ...
                item { OutlinedTextField(state.targetInput, viewModel::setTarget, Modifier.fillMaxWidth(), label = { Text("Mục tiêu cần đạt") }, supportingText = { state.targetInput.toLongOrNull()?.let { Text(it.toVnd()) } }, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number), singleLine = true) }
                ...
                item { OutlinedTextField(state.monthlyInput, viewModel::setMonthly, Modifier.fillMaxWidth(), label = { Text("Số tiền tích lũy mỗi tháng") }, supportingText = { state.monthlyInput.toLongOrNull()?.let { Text(it.toVnd()) } }, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number), singleLine = true) }
