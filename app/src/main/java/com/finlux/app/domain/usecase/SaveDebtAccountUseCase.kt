...
class SaveDebtAccountUseCase @Inject constructor(
    ...
    suspend operator fun invoke(debt: DebtAccount): AppResult<String> {
        ...
        if (debt.isMonthlyRecurring) {
            if (debt.dueDate == null || debt.dueDate !in 1..31) {
                return AppResult.Error("Ngày đến hạn hàng tháng phải từ 1 đến 31")
            }
        } else {
            if (debt.dueDate != null && debt.dueDate !in 1..31) {
                return AppResult.Error("Ngày đến hạn không hợp lệ")
            }
        }
